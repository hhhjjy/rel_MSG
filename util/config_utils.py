...
def get_configs(base_config_dir: str, args: argparse.Namespace, creat_subdir: bool, categories_file: str = 'json') -> Dict[str, Any]:
    """
    ...
    # categories
    # if categories_file == 'json':
    #     categories = json.load(open('/root/autodl-tmp/dataset/Replica/2DSG/val.json'))['categories']
    #     final_config['class_map'] = {category['name']: category['id'] for category in categories}
    #     final_config['inv_class_map'] = {category['id']: category['name'] for category in categories}
    #     final_config['num_classes'] = len(categories)
