...
class TopoMapperHandler(object):
    ...
    def __init__(self, config, video_data_dir, video_id,dataset = None):
        ...
        if dataset == 'Replica_small_split':
        ...
        self.all_object_feat = {}

        # 场景中所有的object feature
    ...
    def map_update(self, batch_data, batch_results):
        ...
        self.all_obj_feature.extend([i['object_embeddings'] for i in list_results])
        self.all_obj_uid.extend([i['detections']['uids'] for i in list_results])
        self.all_obj_label.extend([i['detections']['labels'] for i in list_results])
