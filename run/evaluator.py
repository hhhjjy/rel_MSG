...
class Evaluator(object):
    def __init__(self, video_data_dir, video_id, gt, pred, out_path, inv_class_map,dataset = None):
    ...
    def visualize_det(self, det_type):
        ...
        for frame_id, frame_dict in self.pred['detections'].items():
            ...
            image_source = np.rot90(image_source, k=3)
    ...
    def get_po_iou(self,):
        ...
        self.po_iou_normal = self.po_intersection / self.po_union
    ...
    def get_importance_iou(self,):
        ...
        remaining_indices = [ i for i in range(ori_gt_po_adj.shape[1]) if i not in self.matched_gt_indices]
        reordered_indices = self.matched_gt_indices + remaining_indices
        gt_importance = np.array(self.gt_importance[reordered_indices][:,reordered_indices])
        pred_importance = (self.importance>0.5).astype(int)
        # take the min, use extra 1's as penalizer
        min_col = np.minimum(gt_importance.shape[1], pred_importance.shape[1])
        # print("num objects, gt:", gt_po_adj.shape[1], "pred:", pred_po_adj.shape[1])
        # get intersection and union between the two [:min_col]
        self.importance_intersection = np.sum(np.bitwise_and(gt_importance[:min_col,:min_col], pred_importance[:min_col,:min_col]))
        self.importance_union = np.sum(np.bitwise_or(gt_importance[:min_col,:min_col], pred_importance[:min_col,:min_col]))
        # add residual 1's to the denominator as additinoal penalizer, note that one of the following two terms is 0
        self.importance_residuals = np.sum(gt_importance[min_col:, min_col:]) + np.sum(pred_importance[min_col:, min_col:])
        # print("breakdown", self.po_intersection, self.po_union, self.po_residuals)
        self.importance_iou = self.importance_intersection / (self.importance_union + self.importance_residuals)
        self.importance_iou_normal = self.importance_intersection / self.importance_union   
    ...
    def get_metrics(self,):
        ...
        self.get_pp_iou()
        self.get_po_iou()
        # self.get_importance_iou()
        self.get_graph_iou()
        # self.get_place_recall()
        return {
            'pp_iou': self.pp_iou,
            'po_iou': self.po_iou,
            'po_iou_normal': self.po_iou_normal,
            'graph_iou': self.graph_iou,
            'num_pred_obj': len(self.pred_obj_traj),
            'num_unmatched_pred_obj': len(self.pred_obj_numatched), 
            'num_gt_obj': len(self.gt_obj2col),
            # 'importance_iou': self.importance_iou,
            # 'importance_iou_normal': self.importance_iou_normal,
