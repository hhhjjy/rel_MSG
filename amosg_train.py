...
from datasets.dataset import AppleDataHandler, VideoDataset,VideoDataset_3RScan_split, VideoDataset_3RScan,VideoDataset_Replica_small_split, MultiVideoDataset,VideoDataset_Replica_small, multivideo_collate_fn, arkit_collate_fn, VideoDataset_Replica
...
def train(config):
    ...
    train_data = AppleDataHandler(config["dataset_path"], split=config['train_split'], video_batch_size=config['bs_video'], dataset = '3rscan_split')
    ...
    import pickle
    scan_dict_path = r'D:\SceneGraph\code\MSG\dataset\simple_3rscan\scan_dict.pkl'
    ...
    for e in range(config['num_epochs']):
        ...
        for i, next_video_id in enumerate(train_data):
            ...
            # ccc = time.time()
            # print('collate time1:', ccc-bbb)
            dataloader = DataLoader(dataset, batch_size=config["train_bs"]//config["bs_video"], shuffle=True, 
                                    num_workers=1, collate_fn=multivideo_collate_fn)
            # ddd = time.time()
            # print('collate time2:', ddd-ccc)
            ...
            # ddd = time.time()
            # print('train time:', ddd-ccc)
            # for i in range(len(dataset.datasets)):
            #     if not dataset.datasets[i].has_sam_masks:
            #         torch.save(dataset.datasets[i].sam_masks, dataset.datasets[i].cache_file)
            #         print(f"Save sam masks to {dataset.datasets[i].cache_file}")
            ...
            if training_steps_vid % config['chkpt_every'] == 0 and training_steps_vid > 0:
                ...
                # save_path = os.path.join(config["output_dir"], "checkpoints", f"final.pth")
...
def train_per_video(model, optimizer, dataset, dataloader, device, epoch, loss_params):
    ...
    for batch in dataloader:
        ...
        additional_info = {
            ...
            'scene_num': len(batch['num_per_vid']),
        }
        if 'masks' in additional_info:
            additional_info['masks'] = additional_info['masks'].to(device)
        place_labels = dataset.get_place_labels(batch['image_idx'], batch['num_per_vid'], batch['vid_idx'])
        rel_labels = dataset.get_rel_labels(batch['obj_idx'])

        vid_idx = None
        if 'scene_num' in additional_info and len(images) > 0:
            scene_num = additional_info['scene_num']
            B = len(images)
            images_per_scene = B // scene_num
            vid_idx = torch.arange(scene_num, dtype=torch.long, device=images[0].device).unsqueeze(1).expand(-1, images_per_scene).reshape(-1)
            additional_info['vid_idx'] = vid_idx
        
        # 统一阶段路由: 使用 model.stage 自动选择 forward/loss
        # config['stage'] 优先级高于 config['forward_version'] (兼容旧配置)
        stage = config.get('stage', config.get('forward_version', 'amosg'))
...
from amosg_eval import eval_per_video
...
def eval(config, logger, model, device):
    ...
    for i, next_video_id in enumerate(arkit_data.videos[:100]):
        ...
        # dataset = VideoDataset_Replica(arkit_data.data_split_dir, next_video_id, config, get_transform(config['model_image_size']), split=config['eval_split'], scan_dict=scan_dict)
        # mapper = TopoMapperHandler(config, next_video_path, next_video_id, dataset='Replica')
        
        # dataset = VideoDataset_Replica_small(arkit_data.data_split_dir, next_video_id, config, get_transform(config['model_image_size']), split=config['eval_split'], scan_dict=scan_dict)
        # mapper = TopoMapperHandler(config, next_video_path, next_video_id, dataset='Replica_small')

        # dataset = VideoDataset_Replica_small_split(arkit_data.data_split_dir, next_video_id, config, get_transform(config['model_image_size']), split=config['eval_split'], scan_dict=scan_dict)
        # mapper = TopoMapperHandler(config, next_video_path, next_video_id, dataset='Replica_small_split')

        dataloader = DataLoader(dataset, batch_size=config["eval_bs"], shuffle=False, num_workers=1, collate_fn=arkit_collate_fn)
...
# aomsg_replica.yaml
    # EXPERIMENT = 'aomsg_minival'
    EXPERIMENT = 'simple'
    # get the config
    parser = argparse.ArgumentParser(description="Experiment configurations")
    parser.add_argument('--experiment', default=EXPERIMENT, type=str, help='Name of the experiment config to use')
    ...
    config = get_configs(base_config_dir, args, creat_subdir=True, categories_file = 'json') # for training, always create subdir
    # print(config)
    # fix seed
