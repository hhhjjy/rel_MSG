...
from arkit_dataset import AppleDataHandler, VideoDataset,VideoDataset_3RScan_split, VideoDataset_3RScan,VideoDataset_Replica_small_split, MultiVideoDataset,VideoDataset_Replica_small, multivideo_collate_fn, arkit_collate_fn, VideoDataset_Replica
...
def train_per_video(model, optimizer, dataset, dataloader, device, epoch, loss_params):
    ...
    for batch in dataloader:
        ...
        # ccc = time.time()
        # print('preprocess time:', ccc-bbb)
        results = model(images, additional_info)
        # ddd = time.time()
        # print('model forward time:', ddd-ccc)
        # spec
        total_loss, logs = model.compute_loss(
            results, 
            additional_info, 
            place_labels.type(torch.FloatTensor).to(device),
            rel_labels.type(torch.FloatTensor).to(device),
            weights = loss_params,
        )
        # eee = time.time()
        # print('compute loss time:', eee-ddd)
        optimizer.zero_grad()
        total_loss.backward()
        optimizer.step()
